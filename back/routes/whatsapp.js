...
// Records a successful WhatsApp connection in wabyone_whatsapp_connections.
// Uses session_id (Green API idInstance) as the dedup key so reconnects
// update the existing row rather than creating a second one.
async function upsertWhatsappConnection(instance) {
  try {
    const existing = await db(
      `SELECT id FROM wabyone_whatsapp_connections WHERE session_id = $1 LIMIT 1`,
      [String(instance.id_instance)],
    );

    if (existing.rows.length > 0) {
      await db(
        `UPDATE wabyone_whatsapp_connections
         SET status = 'active',
             whatsapp_number = $2,
             api_token = $3,
             device_name = $4,
      ...
      await db(
        `INSERT INTO wabyone_whatsapp_connections
           (org_id, session_id, api_token, whatsapp_number, provider, device_name, status)
         VALUES ($1, $2, $3, $4, 'green-api', $5, 'active')`,
        [
          instance.org_id,
          String(instance.id_instance),
          instance.api_token_instance,
    ...
    console.error("upsertWhatsappConnection error:", err.message);
  }
}
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (stateInstance === "authorized") {
        ...
        const full = await db(`SELECT * FROM whatsapp_instances WHERE id = $1`, [instanceId]);
        if (full.rows.length) await upsertWhatsappConnection(full.rows[0]);
        return; // done
      }
...
router.post("/webhook/:webhookToken", async (req, res) => {
  ...
  try {
    ...
    if (payload.typeWebhook === "stateInstanceChanged") {
      ...
      if (newState === "authorized") await upsertWhatsappConnection(instance);
      return;
    }
